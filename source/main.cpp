...
const std::vector<Symbol> BroadcastVoiceSyms = {
    ...
    Symbol::FromSignature(
        "\x55\x8B\xEC\xA1****\x83\xEC\x50\x83\x78\x30\x00\x0F\x84****"
        "\x53\x8D\x4D\xD8\xC6\x45\xB4\x01\xC7\x45*****"),
...
const std::vector<Symbol> BroadcastVoiceSyms = {
    Symbol::FromName(
        "_Z21SV_BroadcastVoiceDataP7IClientiPcx"), // 32-bit symbol if it exists
    Symbol::FromSignature(
        "\x55\x48\x8D\x05****"
        "\x48\x89\xE5\x41\x57\x41\x56\x41\x89\xF6\x41\x55\x49"
        "\x89\xFD\x41\x54\x49\x89\xD4\x53\x48\x89\xCB\x48\x81"
        "\xEC****\x48\x8B\x3D****\x48\x39\xC7\x74\x25"), // 64-bit pattern
    Symbol::FromSignature("\x55\x89\xE5\x57\x56\x53\x83\xEC\x6C\xA1\x2A\x2A\x2A"
                          "\x2A\x8B\x75\x14"
                          "\x8B\x7D\x18\x8B\x50\x48\x85\xD2\x74\x05\x80\x3A\x00"
                          "\x75\x10"), // 32-bit pattern
};
#endif
...
typedef void (*SV_BroadcastVoiceData)(IClient *cl, int nBytes, char *data,
                                      int64 xuid);
...
void hook_BroadcastVoiceData(IClient *cl, uint nBytes, char *data, int64 xuid) {
  ...
  // This is (and needs to be) and O(1) operation for how often this function is
  // called. If not in the set, just hit the trampoline to ensure default
  // behavior.
  ...
  if (g_eightbit->broadcastPackets && nBytes > sizeof(uint64_t)) {
    ...
    // Notice that we don't use the conveniently provided one in the voice
    // packet. The client can manipulate that one.

    *(uint64_t *)decompressedBuffer = id64;

    // Transfer the packet data to our scratch buffer
    // This looks jank, but it's to prevent a theoretically malformed packet
    // triggering a massive memcpy
    size_t toCopy = nBytes - sizeof(uint64_t);
    std::memcpy(decompressedBuffer + sizeof(uint64_t), data + sizeof(uint64_t),
                toCopy);

    // Finally we'll broadcast our new packet
    net_handl->SendPacket(g_eightbit->ip.c_str(), g_eightbit->port,
                          decompressedBuffer, nBytes);
  }
  ...
  if (g_eightbit->broadcastRawPcm && nBytes > STEAM_PCKT_SZ) {
    ...
    std::cout << "bytes to transmit: " << nBytes << std::endl;
    IVoiceCodec *codec = codecs[uid];
    char *pcmPayload = pcmSendBuffer + sizeof(uint64_t);
    int decompressed = SteamVoice::DecompressIntoBuffer(
        codec, data, nBytes, pcmPayload,
        sizeof(pcmSendBuffer) - sizeof(uint64_t));
    if (decompressed > 0) {
      *(uint64_t *)pcmSendBuffer = id64;
      net_handl->SendPacket(g_eightbit->ip.c_str(), g_eightbit->pcmPort,
                            pcmSendBuffer, sizeof(uint64_t) + decompressed);
    }
  }
  ...
  if (afflicted_players.find(uid) != afflicted_players.end()) {
    ...
    if (nBytes < STEAM_PCKT_SZ) {
      return detour_BroadcastVoiceData.GetTrampoline<SV_BroadcastVoiceData>()(
          cl, nBytes, data, xuid);
    }

    int bytesDecompressed = SteamVoice::DecompressIntoBuffer(
        codec, data, nBytes, decompressedBuffer, sizeof(decompressedBuffer));
    ...
    int bytesWritten = SteamVoice::CompressIntoBuffer(
        steamid, codec, decompressedBuffer, samples * 2, recompressBuffer,
        sizeof(recompressBuffer), 24000);
    if (bytesWritten <= 0) {
      return detour_BroadcastVoiceData.GetTrampoline<SV_BroadcastVoiceData>()(
          cl, nBytes, data, xuid);
    }
    ...
    return detour_BroadcastVoiceData.GetTrampoline<SV_BroadcastVoiceData>()(
        cl, bytesWritten, recompressBuffer, xuid);
  } else {
    return detour_BroadcastVoiceData.GetTrampoline<SV_BroadcastVoiceData>()(
        cl, nBytes, data, xuid);
  }
...
LUA_FUNCTION_STATIC(eightbit_enableEffect) {
  ...
  if (afflicted_players.find(id) != afflicted_players.end()) {
    ...
    IVoiceCodec *codec = new SteamOpus::Opus_FrameDecoder();
    codec->Init(5, 24000);
    afflicted_players.insert(std::pair<int, std::tuple<IVoiceCodec *, int>>(
        id, std::tuple<IVoiceCodec *, int>(codec, eff)));
  }
  return 0;
}
...
GMOD_MODULE_OPEN() {
  ...
  for (const auto &sym : BroadcastVoiceSyms) {
    sv_bcast = symfinder.Resolve(engine_loader.GetModule(), sym.name.c_str(),
                                 sym.length);
